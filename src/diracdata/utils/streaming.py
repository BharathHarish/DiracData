...
def to_ai_message(text: str, tool_calls: list) -> Any:
    """Rebuild an AIMessage from collected text + tool calls (when the stream had no message object)."""
    from langchain_core.messages import AIMessage
    return AIMessage(content=text or "", tool_calls=tool_calls or [])


def loads_json(text: str) -> Any:
    """Parse a JSON object out of a model's text reply, tolerating ```json fences and surrounding prose."""
    raw = (text or "").strip()
    if raw.startswith("```"):
