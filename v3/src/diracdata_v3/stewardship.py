...
def probe_footprint(engine: Any, sql: str, *, max_cols: int = 16, max_joins: int = 3) -> dict:
...
def _probe_join(engine: Any, left: str, right: str, report: dict) -> None:
    ...
    if r_uniq >= l_uniq:
        (ft, fc, fn), (dt, dc, d_uniq) = (lt, lc, ln), (rt, rc, r_uniq)
    else:
        (ft, fc, fn), (dt, dc, d_uniq) = (rt, rc, rn), (lt, lc, l_uniq)
...
def trust_line(dq: dict, sanity: dict | None = None) -> str:
    ...
    if dq:
        ...
        if rc:
            bits.append("rows " + ", ".join(f"{t}={n:,}" for t, n in list(rc.items())[:3]))
        joins = dq.get("joins") or []
        clean = [j for j in joins if (j.get("orphan_pct") or 0) <= 1 and (j.get("fan_out") or 0) <= 1.5]
        if joins and len(clean) == len(joins):
            bits.append(f"{len(joins)} join(s) clean")
        flags = dq.get("flags") or []
        bits.append(("flags: " + "; ".join(flags[:3])) if flags else "no input flags")
        parts.append("DATA QUALITY: " + ", ".join(bits))
    if sanity:
        sflags = sanity.get("flags") or []
        parts.append("SANITY: " + ("; ".join(sflags[:3]) if sflags
