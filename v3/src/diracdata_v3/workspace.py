...
class Workspace:
    ...
    def load(
        ...
        *,
        ...
        if value_domains_path and Path(value_domains_path).exists():
            ws.value_domains = json.loads(Path(value_domains_path).read_text(encoding="utf-8"))
        # Join graph: derive the canonical edges from the _ref/_record convention, then
        # add any joins LEARNED from verified queries (a join store). BFS composes these
        # single-hop edges into 3-/4-way paths on demand.
        for left_t, left_c, right_t, right_c in _derive_join_edges(table_columns):
            ws._add_edge(left_t, left_c, right_t, right_c, "derived")
        for rec in (join_store.load() if join_store is not None else []):
            ws._add_edge(rec["left_table"], rec["left_col"], rec["right_table"], rec["right_col"], "learned")
        ...
        # remembered experiences join the SAME bank as gold, retrieved identically. They
        # enter as provisional -> usable as precedent (find_examples), not deterministic
        # replay (exact/slot stay gold-only), so an agent answer verified once is not yet
        # replayed blind.
        for rec in (experience_store.load() if experience_store is not None else []):
            ws._add_example("experience", rec.get("question", ""), rec.get("sql", ""), table_columns)

        if semantic_layer_path and Path(semantic_layer_path).exists():
            import yaml
    ...
    def add_experience(self, question: str, sql: str) -> None:
        """Add a remembered experience to the in-memory bank so it is reusable immediately
        in the same session (find_examples surfaces it right away)."""
        self._add_example("experience", question, sql, self._table_columns)

    def has_experience_question(self, question: str) -> bool:
        target = _normalize_question(question)
        return any(ex.source == "experience" and _normalize_question(ex.question) == target for ex in self.examples)
    ...
    def find_examples(self, query: str, *, limit: int = 5) -> list[Example]:
...
def _sample_values(dom: Any, limit: int = 5) -> str:
    ...
    sample = ", ".join(_v(v)[:24] for v in values[:limit])
...
def _format_domain(dom: Any) -> str:
    ...
    hint = "e.g. " + ", ".join(_v(v) for v in values[:8]) + f" ({dom.get('distinct_at_least')}+ distinct)"
